...
__export(provider_exports, {
  ...
  validateNewAssetSettings: () => validateNewAssetSettings
});
...
var import_queue = require("../../utils/queue.js");
...
function getNewAssetSettings(filePath, muxConfig) {
  try {
    ...
    if (muxConfig?.newAssetSettings?.[filePath] || muxConfig?.newAssetSettings?.[normalizedFilePath]) {
      ...
      import_logger.default.info(import_logger.default.label("Asset settings:"), "Using exact path match");
    ...
    if (muxConfig?.newAssetSettings) {
      for (const [pattern, newAssetSettings] of Object.entries(muxConfig.newAssetSettings)) {
        if ((0, import_minimatch.minimatch)(normalizedFilePath, pattern)) {
          import_logger.default.info(import_logger.default.label("Asset settings:"), "Using pattern match");
          return newAssetSettings;
        }
      }
    }
    ...
    import_logger.default.error("Error retrieving asset settings for file:", filePath);
    return void 0;
  }
}
let mux;
...
function initMux() {
  ...
  queue ?? (queue = new import_queue.Queue());
}
...
async function createUploadURL(filePath) {
  try {
    ...
    if (e instanceof Error && "status" in e && e.status === 401) {
      import_logger.default.error("Unauthorized request. Check that your MUX_TOKEN_ID and MUX_TOKEN_SECRET credentials are valid.");
    } else {
