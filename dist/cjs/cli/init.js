...
async function findFileUpTree(filename) {
  let currentDir = process.cwd();
  const root = import_node_path.default.parse(currentDir).root;
  while (currentDir !== root) {
    const filePath = import_node_path.default.join(currentDir, filename);
    try {
  ...
  const rootPath = import_node_path.default.join(root, filename);
  try {
    await (0, import_promises.access)(rootPath);
    return rootPath;
  } catch {
    return null;
  }
